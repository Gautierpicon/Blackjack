...
// Placer une mise et commencer la partie
function placeBet() {
    playerBet = parseInt(betAmountInput.value);
    if (isNaN(playerBet) || playerBet <= 0) {
        messages.textContent = "Veuillez entrer une mise valide.";
    } else {
        messages.textContent = `Vous avez misé ${playerBet} jetons. Bonne chance !`;
        startNewGame();
